...
import { Button, EmptyState, Modal, NextButton, PageHeader } from '../components/ui';
...
export function EvidenceFacts() {
  ...
  return <>
    ...
    <div className="evidence-grid">{evidence.map(e => { const Icon = e.type === 'sheet' ? FileSpreadsheet : e.type === 'pdf' ? FileText : e.type === 'image_check' ? ScanLine : FileImage; return <button className="evidence-card" key={e.id} onClick={() => setSource(e)}><div className={`file-icon ${e.type}`}><Icon size={23} /></div><div><strong>{t(e.name)}</strong><small>{t("· View extracted result")} <ArrowRight size={12} /></small></div></button>; })}</div>
    ...
    <FactReview />
    <Modal open={!!source} onOpenChange={open => !open && setSource(null)} title={t(source?.name ?? 'Evidence')}>{source && <div className="source-preview"><ul>{source.extracted.map(line => <li key={line}>{t(line)}</li>)}</ul></div>}</Modal>
  </>;
}
