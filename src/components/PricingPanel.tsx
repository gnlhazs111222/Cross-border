...
export function PricingPanel({ pricing, compact = false }: { pricing: Pricing; compact?: boolean }) {
  ...
  return <section className={`panel pricing-panel ${compact ? 'compact' : ''}`}><div className="panel-title"><CircleDollarSign size={19} /><h3>{t("Pricing")}</h3><Badge tone="green">{t("Pricing Ready")}</Badge><Badge>{t("Snapshot Version:")} {pricing.version}</Badge></div><div className="pricing-layout">{compact ? <details className="cost-disclosure"><summary>{t("Cost assumptions")}</summary>{costGrid}</details> : costGrid}<div className="suggested-price"><span>{t("Suggested price")}</span><strong>{money(pricing.suggestedPrice!)}</strong><small>{t("Estimated unit profit:")} {money(pricing.suggestedPrice! - pricing.supplierCost - pricing.shipping - pricing.duty - pricing.platformCost)}</small></div></div></section>;
}
