...
export function PricingPanel({ pricing, compact = false }: { pricing: Pricing; compact?: boolean }) {
  ...
  return <section className={`panel pricing-panel ${compact ? 'compact' : ''}`}><div className="panel-title"><CircleDollarSign size={19} /><h3>{t("Demo Pricing Snapshot")}</h3><Badge>{t("Snapshot Version:")} {pricing.version}</Badge></div><div className="pricing-layout"><div className="cost-grid">{costs.map(([label, amount]) => <div key={label}><span>{t(label)}</span><strong>{money(amount)}</strong></div>)}</div><div className="suggested-price"><span>{t("Suggested price")}</span><strong>{money(pricing.suggestedPrice!)}</strong><small>{t("Estimated unit profit:")} {money(pricing.suggestedPrice! - pricing.supplierCost - pricing.shipping - pricing.duty - pricing.platformCost)}</small></div></div><p className="footnote">{t("Fixed demo assumptions · no live logistics, tax or marketplace fee lookup. Internal costs stay out of consumer copy.")}</p></section>;
}
