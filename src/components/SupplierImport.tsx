...
function ImportResults({ report }: { report: ImportReport }) {
  ...
  const issueText = (issue: ImportIssue) => {
    const messages = {
      ...
      format: 'Unsupported format: {field}. Check the sample file for the accepted values.',    };
...
export function SupplierImport({ onDatasetChange }: { onDatasetChange: () => void }) {
  ...
  return <>
    <section className="panel supplier-import-panel">
      <div className="supplier-import-toolbar"><div><h2>{t('Supplier materials')}</h2></div><div className="supplier-import-actions">
      ...
      <div className="supplier-file-links"><span>{t('Download sample:')}</span><a href="/demo/prismlaunch-supplier-demo.xlsx" download><Download size={13} />Excel (.xlsx)</a><a href="/demo/prismlaunch-supplier-demo.csv" download><Download size={13} />CSV (.csv)</a></div>
      ...
      {batch && <div className="import-batch-summary" data-testid="import-batch-summary">
        ...
        {attachment && <div className="notice" data-testid="attachment-result-panel">{t('Attached {count} file(s) to products.', { count: attachment.attached })}{attachment.missing.length > 0 && <> {t('{count} referenced file(s) are not in the chosen folder.', { count: attachment.missing.length })}</>}{attachment.unreferenced.length > 0 && <> {t('{count} chosen file(s) are not referenced by any row.', { count: attachment.unreferenced.length })}</>}</div>}
    ...
    <Modal open={open} onOpenChange={value => !busy && setOpen(value)} title={t('Import Supplier File')} description={t('Fixed template only. No automatic column guessing. Maximum 2 MB and 500 data rows.')} wide>
      ...
      {preview && folder.length > 0 && (() => {
        ...
        return <div className="notice" data-testid="asset-preview">
          ...
          {match.missing.length > 0 && <p>{t('{count} referenced file(s) are not in the chosen folder.', { count: match.missing.length })}</p>}
          {match.unreferenced.length > 0 && <p>{t('{count} chosen file(s) are not referenced by any row.', { count: match.unreferenced.length })}</p>}
          {match.grouped.length > 0 && <p>{t('{count} row(s) claim several files, please confirm.', { count: match.grouped.length })}</p>}
        </div>;
      })()}
      ...
      {attachment && <div className="notice" data-testid="attachment-result">{t('Attached {count} file(s) to products.', { count: attachment.attached })}{attachment.missing.length > 0 && <> {t('{count} referenced file(s) are not in the chosen folder.', { count: attachment.missing.length })}</>}{attachment.unreferenced.length > 0 && <> {t('{count} chosen file(s) are not referenced by any row.', { count: attachment.unreferenced.length })}</>}{attachment.grouped.length > 0 && <> {t('{count} row(s) claim several files, please confirm.', { count: attachment.grouped.length })}</>}</div>}
        <details className="template-columns"><summary>{t('Supported template columns')}</summary><code>{mockApi.getSupplierTemplate().join(', ')}</code><p>{t('Use exactly these case-sensitive column names, in any order. XLSX must contain one worksheet; CSV must use UTF-8 and commas. Empty packaging fields are allowed but block pricing.')}</p><p>{t('The quality report travels with the file: fill reportNo, reportResult, reportValidUntil, reportCapacityMl and reportMaterial and the report is on file, so the evidence checks never ask for it again. A blank reportNo means this SKU has no report.')}</p></details>
