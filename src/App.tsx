...
export default function App() {
  ...
  useEffect(() => { let active = true; void (async () => { try { const catalog = await mockApi.getCatalog(); const ready = await mockApi.ready(); if (active) { setProducts(catalog); setState(ready); } } catch { if (active) notify('Could not load demo data. Reload the page to retry.', true); } finally { if (active) setLoading(false); } })(); return () => { active = false; }; }, [notify]);
  ...
  const act = async (label: string, action: () => Promise<DemoState>, message: string) => {
    ...
    try { setState(await action()); notify(message); return true; }
