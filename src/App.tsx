...
export default function App() {
  ...
  return <DemoContext.Provider value={{ state, products, busy, act, navigate, setState, notify }}><div className="app-shell"><a className="skip-link" href="#main">{t("Skip to workspace")}</a><aside className="sidebar"><a href="#materials" className="brand" aria-label="海淘集市" onClick={e => { e.preventDefault(); navigate('materials'); }}><img src="/haitao-market-logo.png" alt="" width="35" height="35" /><span>海淘集市</span></a><nav aria-label={t("Workspaces")}>{workspaces.map(({ id, label, Icon }, i) => <button key={id} className={`nav-item ${state.workspace === id ? 'active' : ''}`} aria-current={state.workspace === id ? 'page' : undefined} onClick={() => navigate(id)} disabled={!!busy}><Icon size={19} /><span>{t(label)}</span>{completed[i] ? <Check size={14} className="nav-check" /> : <small aria-hidden="true">{String(i + 1).padStart(2, '0')}</small>}</button>)}</nav><div className="sidebar-bottom"><Button variant="ghost" disabled={!!busy || loading} busy={busy === 'reset'} onClick={() => void act('reset', () => mockApi.reset(), 'Demo reset. Start again from Materials.')}><RotateCcw size={16} />{t("Reset Demo")}</Button><div className="sidebar-footer"><span className="user-avatar">海</span><div><strong>{t("Haitao Market Demo")}</strong><span>{t("Local workspace · v1.0")}</span></div></div></div></aside>
    <div className="main-shell"><header className="topbar"><div className="breadcrumb"><strong>{t(workspaces[index].label)}</strong></div><div className="topbar-badges"><DemoCapabilities /><UnitSwitcher /><LanguageSwitcher /><UserMenu /></div></header><div className="contextbar"><div>{state.task ? <span className="context-identity"><span>{t("Task")}</span><code>{state.task.id}</code></span> : <span>{t("No active launch task")}</span>}{state.selectedSku && <><ChevronRight size={13} /><span className="context-identity"><span>SKU</span><code>{state.selectedSku}</code></span></>}</div><span className="state-indicator"><span className="status-dot" />{t(stageLabel(state))}</span></div>
