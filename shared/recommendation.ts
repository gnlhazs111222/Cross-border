...
export function ruleRank(products: Product[], task: Task, context?: RecommendationContext): Recommendation[] {
  ...
  return eligible.map(p => {
    ...
    if (p.visual === 'bottle' && target) { const delta = Math.abs(p.capacity - target) / target; if (delta <= 0.1) reasons.push(`${p.capacity}ml capacity matches the target`); else { score -= Math.min(30, Math.round(delta * 24)); deductions.push(`Capacity: ${p.capacity}ml; requested about ${Math.round(target)}ml`); } }
    else if (p.visual === 'bottle' && (large || small)) { const distance = max === min ? 0 : (large ? max - p.capacity : p.capacity - min) / (max - min); score -= Math.round(distance * 24); reasons.push(`${p.capacity}ml capacity considered for the ${large ? 'large' : 'small'} capacity preference`); }
