...
`FactCard.evidence` 只保留供应商表格一条（`EV-001`）。此前“表格 / 规格 PDF / 商品图各一条”中的 PDF 与图片卡已删除：文档和图片解析没有实现，不再展示并不存在的提取结果。真实上传的资产走 `ProductAsset` 与快照的 `assets` 字段，图片由多模态图片文字核对模块接入（另出一张 `image_check` 证据卡）。
