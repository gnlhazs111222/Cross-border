...
test('Qwen output persists metadata, passes explicit rules review, publishes, and reuses cached generation', async () => {
  ...
  assert.equal(w.listings.amazon.generationMode, 'qwen'); assert.equal(w.listings.amazon.riskDemoInjected, false); assert.equal(w.listings.amazon.generation.promptVersion, 'listing-qwen-v3'); assert.ok(w.listings.amazon.generation.aiCallId);
  ...
  const audit = await db.aiCall.findUniqueOrThrow({ where: { id: w.listings.shopify.generation.aiCallId } }); assert.equal(audit.purpose, 'listing_generation'); assert.equal(audit.promptVersion, 'listing-qwen-v3'); assert.ok(audit.inputHash); assert.equal(audit.totalTokens, 50);
